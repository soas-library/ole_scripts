...
my $filecount = 0;
my $days;
my $file_prefix = "vufind_full-*";
my $xml_prefix = "201*";
my $source_server_name = "william";

# Note different directory locations for different OLE servers: comment out as appropriate

#my $EXPORT_DIR = "/usr/local/ole15/kuali/main/prd/olefs-webapp/work/staging/export/vufind_full_export/"; # Specific to james.lis.soas.ac.uk
my $EXPORT_DIR = "/home/ole15/kuali/main/qa_william/olefs-webapp/work/staging/export/vufind_full_export/"; # Specific to william.lis.soas.ac.uk
#my $EXPORT_DIR = "/usr/local/ole15/kuali/main/dev_lisbet/olefs-webapp/work/staging/export/vufind_full_export/"; # Specific to lisbet.lis.soas.ac.uk
...
sub remove_bib_export_files
{
	...
	while ($file_prefix = readdir DIR)
	{
		...
		my $mtime = (stat "$EXPORT_DIR/$file_prefix")[9];
		if ($TODAY - $wktime > $mtime)
		{
			$filecount++;
			print LOGFILE "$datestamp - Removing $EXPORT_DIR/$file_prefix as older than $days days\n";
			unlink "$EXPORT_DIR/$file_prefix";
		}
	}
	close DIR;
}
